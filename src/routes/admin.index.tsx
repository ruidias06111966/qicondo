    ...
    <div className="p-6 lg:p-8 space-y-6 max-w-7xl mx-auto">
      <header>
        ...
        <p className="text-sm text-muted-foreground">
          Métricas, assinaturas e gestão de todas as empresas.
        </p>
      </header>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
        <Kpi
          label="Empresas"
          value={m?.total_empresas}
          icon={Building2}
          sub={`${m?.empresas_ativas ?? 0} ativas`}
        />
      ...
      <section className="bg-background border border-border rounded-xl">
        ...
        {loading ? (
          <div className="p-10 text-center">
            <Loader2 className="animate-spin inline text-muted-foreground" />
          </div>
        ) : filtered.length === 0 ? (
                ...
                {filtered.map((e) => (
                  <tr key={e.id} className="border-t border-border hover:bg-muted/30">
                    ...
                    <td className="p-3 text-right whitespace-nowrap">
                      ...
                      <button
                        onClick={async () => {
                          const motivo = e.suspenso
                            ? null
                            : prompt("Motivo da suspensão (opcional):") || null;
                          const r = await safeCall(
                            suspFn({
                              data: { condominio_id: e.id, suspenso: !e.suspenso, motivo },
                            }),
                          );
                          if (r) {
                            toast.success(e.suspenso ? "Reativada" : "Suspensa");
                            reload();
                          }
                        }}
                        className="p-1.5 rounded hover:bg-muted"
                      ...
                      >
                        {e.suspenso ? <Play size={14} /> : <Pause size={14} />}
                      </button>
                      <button
                        onClick={async () => {
                          ...
                          if (!c) return;
                          const r = await safeCall(
                            delFn({ data: { condominio_id: e.id, confirmar_nome: c } }),
                          );
                          if (r) {
...
function EditarAssinaturaModal({
  empresa,
  onClose,
  onSaved,
}: {
  ...
  return (
    <div
      className="fixed inset-0 bg-background/80 backdrop-blur grid place-items-center z-50 p-4"
      onClick={onClose}
    >
      <div
      ...
      >
        ...
        <p className="text-xs text-muted-foreground">
          Valor da assinatura é sempre sob consulta — combinado fora do sistema.
        </p>
        <div className="space-y-3 text-sm">
          <Field label="Plano">
            <select
              ...
              className="w-full bg-muted rounded px-3 py-2 border border-border"
            >
              {PLANOS.map((p) => (
        ...
        <div className="flex gap-2 justify-end">
          <button onClick={onClose} className="px-3 py-2 text-sm rounded hover:bg-muted">
            Cancelar
          </button>
          <button
            disabled={saving}
