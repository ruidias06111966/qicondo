...
const STATUS_INFO: Record<Status, { label: string; classe: string; icon: typeof Clock }> = {
  ...
  aguardando_morador: {
    ...
    classe: "bg-purple-100 text-purple-700 dark:bg-purple-900/40 dark:text-purple-300",
    icon: Clock,
  },
  resolvida: {
    label: "Resolvida",
    classe: "bg-emerald-100 text-emerald-700 dark:bg-emerald-900/40 dark:text-emerald-300",
    icon: CheckCircle2,
  },
...
function OcorrenciasPage() {
    ...
    const r = await safeCall(
      Promise.all([
        ...
        supabase
          ...
          .order("numero"),
        user
          ? supabase.from("unidade_moradores").select("unidade_id").eq("user_id", user.id)
  ...
  const lista = useMemo(() => {
    return ocorrencias.filter((o) => {
      if (filtro === "abertas" && (o.status === "fechada" || o.status === "cancelada"))
        return false;
      if (filtro === "minhas" && o.aberta_por !== user?.id) return false;
      if (busca) {
        const q = busca.toLowerCase();
        const txt =
          `${o.titulo} ${o.descricao} ${o.atribuido_a ?? ""} ${unidadeLabel(o.unidade_id)}`.toLowerCase();
  ...
  const stats = useMemo(
    () => ({
      ...
      urgentes: ocorrencias.filter(
        (o) => (o.status === "aberta" || o.status === "em_andamento") && o.prioridade === "urgente",
      ).length,
    }),
    [ocorrencias],
  );
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 mb-6">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Chamados, manutenções e reclamações do condomínio.
          </p>
        </div>
        <button
          onClick={() => setShowNova(true)}
          className="inline-flex items-center gap-2 rounded-lg bg-primary px-4 py-2 text-sm font-semibold text-primary-foreground hover:bg-[var(--color-primary-deep)]"
        >
        ...
        <div className="flex gap-1 rounded-lg border border-border bg-background p-1">
          {(["abertas", "minhas", "todas"] as const).map((f) => (
            <button
              key={f}
              onClick={() => setFiltro(f)}
              className={`px-3 py-1.5 text-xs font-semibold rounded-md ${filtro === f ? "bg-primary text-primary-foreground" : "text-muted-foreground hover:text-foreground"}`}
            >
          ...
          {lista.map((o) => {
            ...
            return (
              <button
                key={o.id}
                onClick={() => abrir(o)}
                className="w-full text-left bg-background border border-border rounded-xl p-4 hover:border-primary/40 hover:shadow-sm transition-all"
              >
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex flex-wrap items-center gap-2 mb-1">
                      ...
                      <span
                        className={`text-[10px] px-2 py-0.5 rounded-full font-semibold ${PRIOS[o.prioridade].classe}`}
                      >
                        {PRIOS[o.prioridade].label}
                      </span>
                      <span
                        className={`text-[10px] px-2 py-0.5 rounded-full font-semibold inline-flex items-center gap-1 ${SI.classe}`}
                      >
                    ...
                    <div className="flex flex-wrap gap-x-3 gap-y-1 mt-2 text-[11px] text-muted-foreground">
                      ...
                      {o.nota_satisfacao && (
                        <span className="inline-flex items-center gap-0.5 text-amber-500">
                          {Array.from({ length: o.nota_satisfacao }).map((_, i) => (
...
function NovaOcorrencia({
  ...
  const opcoesUnidades = podeGerenciar
    ? unidades
    : unidades.filter((u) => minhasUnidades.includes(u.id));
      ...
      <form onSubmit={submit} className="space-y-3">
        ...
        <div className="grid grid-cols-2 gap-3">
          <Field label="Categoria">
            <select
              ...
              className="input"
            >
              {(Object.keys(CATS) as Categoria[]).map((c) => (
          ...
          <Field label="Prioridade">
            <select
              ...
              className="input"
            >
              {(Object.keys(PRIOS) as Prioridade[]).map((p) => (
        ...
        <Field label="Local (opcional)">
          <input
            ...
            placeholder="Ex.: Garagem subsolo, hall do 3º andar"
            className="input"
          />
        </Field>
        ...
        <div className="flex gap-2 justify-end pt-2">
          ...
          <button
            type="submit"
            disabled={enviando}
            className="inline-flex items-center gap-2 px-4 py-2 text-sm font-semibold rounded-lg bg-primary text-primary-foreground disabled:opacity-50"
          >
...
function DetalheOcorrencia({
  ...
  useEffect(() => {
    void carregarComentarios(); /* eslint-disable-next-line */
  }, [ocorrencia.id]);
    ...
    <Modal onClose={onClose} title={ocorrencia.titulo} wide>
      <div className="space-y-4">
        ...
        <div className="flex flex-wrap gap-2">
          <span
            className={`text-xs px-2 py-1 rounded-full font-semibold ${PRIOS[ocorrencia.prioridade].classe}`}
          >
            {PRIOS[ocorrencia.prioridade].label}
          </span>
          <span
            className={`text-xs px-2 py-1 rounded-full font-semibold inline-flex items-center gap-1 ${SI.classe}`}
          >
        ...
        <div className="bg-muted/30 rounded-lg p-4 space-y-2 text-sm">
          ...
          <div className="flex flex-wrap gap-x-4 gap-y-1 text-xs text-muted-foreground pt-2 border-t border-border">
            <span>
              <strong>Unidade:</strong> {unidadeLabel}
            </span>
            {ocorrencia.local && (
              <span>
            ...
            <span>
              <strong>Aberta:</strong> {dateTimeBR(ocorrencia.created_at)}
            </span>
            {ocorrencia.atribuido_a && (
              <span>
                <strong>Atribuída a:</strong> {ocorrencia.atribuido_a}
              </span>
            )}
          </div>
          {ocorrencia.foto_url && (
            ...
            <div className="mt-2 pt-2 border-t border-border flex items-center gap-1 text-amber-500">
              ...
              {Array.from({ length: 5 }).map((_, i) => (
                <Star
                  key={i}
                  size={14}
                  fill={i < ocorrencia.nota_satisfacao! ? "currentColor" : "none"}
                />
              ))}
            </div>
          )}
        ...
        {podeGerenciar && ocorrencia.status !== "fechada" && ocorrencia.status !== "cancelada" && (
          <div className="flex flex-wrap gap-2">
            ...
            {(ocorrencia.status === "em_andamento" || ocorrencia.status === "aberta") && (
              <button
                onClick={() => mudarStatus("aguardando_morador")}
                className="text-xs px-3 py-1.5 rounded-lg border border-border hover:bg-muted"
              >
                Aguardar morador
              </button>
            ...
            <button
              ...
              className="text-xs px-3 py-1.5 rounded-lg text-destructive border border-destructive/40 hover:bg-destructive/10"
            >
              Cancelar
            </button>
          </div>
        ...
        {souAutor && ocorrencia.status === "resolvida" && !ocorrencia.nota_satisfacao && (
          <div className="border border-amber-500/40 bg-amber-50 dark:bg-amber-900/10 rounded-lg p-3">
            ...
            <button
              ...
              className="text-xs px-3 py-1.5 rounded-lg bg-amber-500 text-white hover:bg-amber-600 inline-flex items-center gap-1"
            >
          ...
          <div className="space-y-2 max-h-64 overflow-y-auto">
            {carregando ? (
              ...
              comentarios.map((c) => (
                <div
                  key={c.id}
                  className={`p-2.5 rounded-lg text-sm ${c.interno ? "bg-amber-100 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-900/40" : "bg-muted/40"}`}
                >
                  {c.interno && (
      ...
      {showResolver && (
        <ResolverModal
          ...
          onOk={(resolucao) => {
            ...
            void mudarStatus("resolvida", resolucao);
          }}
        />
      )}
      {showAvaliar && (
...
function AtribuirModal({
  ...
  const submit = async () => {
    ...
    const { error } = await supabase.rpc("atribuir_ocorrencia", {
      _ocorrencia_id: ocorrenciaId,
      _atribuido_a: valor.trim(),
    });
    setLoading(false);
    if (error) {
      toast.error(ERROS[error.message] ?? error.message);
      ...
      <div className="space-y-3">
        <Field label="Responsável (zelador, prestador, empresa...)">
          <input
            ...
            placeholder="Ex.: João (zelador), Hidrotec Ltda"
            className="input"
            autoFocus
          />
        </Field>
...
function Modal({
  ...
  return (
    ...
    >
      <div
        className={`bg-background rounded-2xl shadow-xl w-full ${wide ? "max-w-3xl" : "max-w-lg"} max-h-[90vh] overflow-y-auto`}
