...
function OnboardingPage() {
  ...
  async function criarUnidades() {
    if (!condId) return;
    if (qtd < 1 || qtd > 200) {
      toast.error("Entre 1 e 200 unidades");
      return;
    }
    setBusy(true);
  ...
  async function entrarComCodigo() {
    ...
    setBusy(true);
    const { data, error } = await supabase.rpc("join_condominio_by_code", {
      _codigo: codigo.trim(),
    });
    setBusy(false);
    if (error) {
      const msg = error.message.includes("codigo_nao_encontrado")
      ...
      <main className="max-w-2xl mx-auto px-6 py-12">
        {step === "intro" && (
          <div>
            <p className="text-sm text-muted-foreground">
              Olá, {(profile?.nome_completo || user.email || "").split(" ")[0]}
            </p>
            <h1 className="font-display text-4xl font-extrabold mt-2">Como vai usar o QiCond?</h1>
            <p className="mt-3 text-muted-foreground">
              Escolha o ponto de partida. Pode sempre adicionar mais empresas ou aceitar novos
              convites depois.
            </p>
            ...
            <div className="mt-8 grid sm:grid-cols-2 gap-4">
              <button
                ...
                className="text-left p-6 rounded-2xl border-2 border-border hover:border-primary bg-background transition-colors group"
              >
                ...
                <p className="mt-1 text-sm text-muted-foreground">
                  Crio a empresa e fico como Administrador. Depois convido a equipa.
                </p>
                <span className="mt-3 inline-flex items-center text-sm text-primary font-semibold group-hover:gap-2 gap-1 transition-all">
              ...
              >
                ...
                <p className="mt-1 text-sm text-muted-foreground">
                  Já tenho o código público partilhado pelo Administrador da empresa.
                </p>
                <span className="mt-3 inline-flex items-center text-sm text-primary font-semibold group-hover:gap-2 gap-1 transition-all">
          ...
          <Card title="Dados da empresa" subtitle="Pode editar tudo depois em Empresa.">
            <div className="grid sm:grid-cols-2 gap-3">
              <Input
                ...
                value={cond.nome}
                onChange={(v) => setCond({ ...cond, nome: v })}
                className="sm:col-span-2"
              />
              <Input
                label="CNPJ"
                value={cond.cnpj}
                onChange={(v) => setCond({ ...cond, cnpj: v })}
              />
              <Input
                label="WhatsApp da empresa"
                value={cond.whatsapp_numero}
                onChange={(v) => setCond({ ...cond, whatsapp_numero: v })}
              />
              <Input
                label="Responsável*"
                value={responsavel}
              ...
              <Input
                label="Endereço"
                value={cond.endereco}
                onChange={(v) => setCond({ ...cond, endereco: v })}
                className="sm:col-span-2"
              />
              <Input
                label="Cidade"
                value={cond.cidade}
                onChange={(v) => setCond({ ...cond, cidade: v })}
              />
              <Input
                label="UF"
                maxLength={2}
                value={cond.estado}
                onChange={(v) => setCond({ ...cond, estado: v.toUpperCase() })}
              />
            ...
            <p className="mt-3 text-xs text-muted-foreground">
              Ficará automaticamente registado como <b>Administrador</b> desta empresa. Depois
              convida a equipa em <b>Equipa</b>. O plano escolhido acima é uma indicação para a
              nossa equipa comercial e é activado após a contratação.
            </p>
            <Actions>
        ...
        {step === "criar-unidades" && (
          <Card
            ...
            subtitle="Quantas unidades existem? Você pode editar valores e blocos depois."
          >
            <div className="grid sm:grid-cols-2 gap-3">
              <Input
                label="Quantidade*"
                type="number"
                value={String(qtd)}
                onChange={(v) => setQtd(Number(v) || 0)}
              />
              <Input
                label="Taxa mensal padrão (R$)"
                placeholder="350,00"
                value={taxa}
                onChange={setTaxa}
              />
            ...
            <p className="mt-3 text-xs text-muted-foreground">
              Vamos criar unidades numeradas de 01 até {String(qtd).padStart(2, "0")}.
            </p>
            <Actions>
              <button onClick={() => setStep("pronto")} className="btn-ghost">
                Pular por agora
              </button>
        ...
        {step === "entrar" && (
          <Card
            ...
            subtitle="Use o código público partilhado pelo Administrador (ex: COND-A1B2C3)."
          >
            <Input
            ...
            />
            <p className="mt-3 text-xs text-muted-foreground">
              Não tem código? Peça ao Administrador para enviar um <b>link de convite</b> com o seu
              perfil já definido.
            </p>
            <Actions>
              <button onClick={() => setStep("intro")} className="btn-ghost">
            ...
            <div className="flex items-start gap-3 p-4 bg-primary/5 rounded-lg">
              ...
              <div className="text-sm">
                ...
                <ul className="mt-2 list-disc pl-5 text-muted-foreground space-y-1">
                  <li>
                    Convidar a equipa em <b>Utilizadores</b>
                  </li>
                  <li>
                    Configurar dados bancários em <b>Financeiro → Pagamentos</b>
                  </li>
