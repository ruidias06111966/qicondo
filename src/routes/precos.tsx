  ...
  head: () => ({
    meta: [
      { title: "Planos — QiCond" },
      {
        name: "description",
        content:
          "Planos sob consulta para empresas de qualquer dimensão. Contrate por empresa, gerencie utilizadores com perfis dedicados, sem fidelidade.",
      },
...
const faqs = [
  {
    q: "Quanto custa?",
    a: "Os planos são sob consulta. Os valores variam conforme o número de utilizadores e os módulos contratados. Fale connosco e enviamos uma proposta no mesmo dia.",
  },
  {
    q: "Como funciona a contratação?",
    a: "O contrato é com a empresa, não com cada utilizador. Você contrata um plano, define o administrador, e ele convida a equipa (Financeiro, Gestor, Vendedor, Comercial, Contador, Consulta).",
  },
  {
    q: "Tem período de avaliação?",
    a: "Sim. Damos 30 dias para experimentar o sistema com a sua equipa, sem cartão. No fim escolhe o plano que faz mais sentido.",
  },
  {
    q: "Os dados de uma empresa misturam-se com os de outra?",
    a: "Nunca. Cada empresa é um tenant isolado. Um utilizador da Empresa A nunca consegue ver dados da Empresa B, mesmo que use o mesmo e-mail noutra empresa.",
  },
  {
    q: "Quem emite os boletos? E o PIX?",
    a: "Cada empresa recebe boletos e PIX direto na própria conta bancária. O QiCond gera o instrutivo e envia ao destinatário pelo WhatsApp; não somos intermediários financeiros — o dinheiro nunca passa pela plataforma.",
  },
  {
    q: "Como faço a migração de outro sistema?",
    a: "Fornecemos planilha modelo e a nossa equipa ajuda com a importação de utilizadores, unidades e histórico. Sem custo adicional em qualquer plano.",
  },
  {
    q: "É compatível com a LGPD?",
    a: "Sim. Política de privacidade clara, consentimento registado, controlo de acesso por perfil, logs de auditoria e exportação dos dados a qualquer momento.",
  },
  {
    q: "Posso cancelar quando quiser?",
    a: "Sim. Sem fidelidade, sem multa de saída. Cancela direto pelo painel ou falando connosco.",
  },
];
    ...
    <SiteLayout>
      <section className="mx-auto max-w-7xl px-4 md:px-8 pt-16 pb-10 text-center">
        ...
        <p className="mt-5 text-muted-foreground max-w-2xl mx-auto">
          Contratação por empresa, com utilizadores ilimitados em todos os planos. Sem fidelidade,
          sem taxa de setup, 30 dias para experimentar.
        </p>
      </section>
      ...
      <section className="mx-auto max-w-7xl px-4 md:px-8 pb-16 grid md:grid-cols-3 gap-5">
        {plans.map((p) => (
          ...
          >
            ...
            <div
              className={`text-sm font-semibold mb-2 ${p.highlight ? "text-white/80" : "text-muted-foreground"}`}
            >
              {p.name}
            </div>

            <div className="mb-1">
              <span className="font-display font-extrabold text-4xl md:text-5xl tracking-tight">
                Sob consulta
              </span>
            </div>
            <div
              className={`text-xs mb-5 ${p.highlight ? "text-white/75" : "text-muted-foreground"}`}
            >
              Faturação por empresa
            </div>

            <p className={`text-sm mb-5 ${p.highlight ? "text-white/85" : "text-foreground"}`}>
              {p.desc}
            </p>

            <div
              className={`rounded-lg px-3 py-2.5 mb-5 text-xs font-medium ${p.highlight ? "bg-white/10 text-white" : "bg-muted/60 text-foreground"}`}
            >
              <div>{p.limites.empresas}</div>
              <div className={p.highlight ? "text-white/80" : "text-muted-foreground"}>
                {p.limites.utilizadores}
              </div>
            </div>
              ...
              {p.features.map((f) => (
                <li key={f.label} className="flex items-start gap-2 text-sm">
                  {f.has ? (
                    <CheckCircle2
                      size={16}
                      className={`shrink-0 mt-0.5 ${p.highlight ? "text-white" : "text-success"}`}
                    />
                  ) : (
                    <X
                      size={16}
                      className={`shrink-0 mt-0.5 ${p.highlight ? "text-white/50" : "text-muted-foreground/40"}`}
                    />
                  )}
                  <span
                    className={
      ...
      <section className="mx-auto max-w-3xl px-4 md:px-8 pb-20">
        <h2 className="font-display font-extrabold text-3xl md:text-4xl text-center mb-10">
