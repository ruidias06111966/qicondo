...
export const Route = createFileRoute("/status")({
  head: () => ({
    meta: [
      { title: "Status — QiCond" },
      { name: "description", content: "Status em tempo real dos serviços QiCond." },
    ],
  }),
    ...
    <SiteLayout>
      <section className="mx-auto max-w-4xl px-4 md:px-8 pt-16 pb-12">
        <div className="text-center mb-10">
          ...
          <h1 className="mt-5 font-display font-extrabold text-4xl md:text-5xl">
            Status do sistema
          </h1>
        ...
        <div className="rounded-2xl border border-border bg-surface overflow-hidden">
          {services.map((s, i) => (
            <div
              key={s.name}
              className={`flex items-center justify-between p-5 ${i > 0 ? "border-t border-border" : ""}`}
            >
              ...
              <div className="text-sm text-muted-foreground">
                Operacional · <span className="text-success font-semibold">{s.uptime}</span>
              </div>
            </div>
          ))}
        </div>
